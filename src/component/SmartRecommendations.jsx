...
console.log(handle);
  ...
  useEffect(() => {
      ...
      try {
        ...
        const recs = problems
          .filter((p) => {
            ...
            return matchesWeakTag && !solvedSet.has(id) && inDifficulty;
          })
    ...
    fetchData();
  }, [handle, difficultyRange, suggestedCount]);
 ...
 return (
   <div className=" mx-auto p-6 font-sans">
     ...
     {!loading && !error && recommended.length > 0 && (
       <div className="mb-6">
         <h3 className="text-lg font-semibold text-gray-800 mb-2">Weak Tags</h3>
         <div className="flex flex-wrap gap-2">
           {[
             ...
             [
               ...
               <span
                 key={tag}
                 className="px-3 py-1 rounded-full text-sm bg-blue-100 text-blue-700 font-medium"
               >
                 #{tag}
               </span>
             ))
     ...
     {!loading && !error && recommended.length === 0 && (
       <div className="p-4 border border-dashed border-gray-300 rounded-lg bg-gray-50 text-gray-600">
         No recommendations found in this range. Try to adjust the difficulty or
         solve more problems to generate weak tags.
       </div>
     )}
